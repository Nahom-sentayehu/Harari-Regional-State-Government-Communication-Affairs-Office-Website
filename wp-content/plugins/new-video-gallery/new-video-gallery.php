...
if ( ! class_exists( 'New_Video_Gallery' ) ) {
	...
	class New_Video_Gallery {
		...
		public function _New_Video_Gallery() {
			$labels = array(
				'name'               => __( 'Video Gallery', 'Post Type General Name', 'new-video-gallery' ),
				'singular_name'      => __( 'Video Gallery', 'Post Type Singular Name', 'new-video-gallery' ),
		...
		public function _vg_shortcode_left_metabox( $post ) { ?>
			<p class="input-text-wrap">
				<input type="text" name="VIDEOCopyShortcode" id="VIDEOCopyShortcode" value="<?php echo '[VDGAL id=' . esc_attr( $post->ID ) . ']'; ?>" readonly style="height: 60px; text-align: center; width:100%;  font-size: 24px; border: 2px dashed;">
				<p id="vg-copy-code"><?php esc_html_e( 'Shortcode copied to clipboard!', 'new-video-gallery' ); ?></p>
				<p style="margin-top: 10px"><?php esc_html_e( 'Copy & Embed shotcode into any Page/ Post / Text Widget to display gallery.', 'new-video-gallery' ); ?></p>
			</p>
			...
			<!--Add New Image Button-->
				<div class="file-upload">
					<div class="image-upload-wrap">
						...
						<div class="drag-text">
							<h3><?php esc_html_e( 'ADD Video Banner', 'new-video-gallery' ); ?></h3>
						</div>
					</div>
				</div>
			</div>
		...
		public function _vg_ajax_callback_function( $id ) {
			...
			<li class="slide">
				...
				<select id="slide-type[]" name="slide-type[]" style="width: 100%;" placeholder="Image Title" value="<?php echo esc_html( $image_type ); ?>" >
					<option value="y" 
					<?php
					if ( $image_type == 'y' ) {
				...
				<select id="poster-type[]" name="poster-type[]" style="width: 100%;" value="<?php echo esc_html( $poster_type ); ?>">
					...
					<option value="internal"><?php esc_html_e( 'Use Above Poster', 'new-video-gallery' ); ?></option>
					<option value="youtube"><?php esc_html_e( 'Fetch YouTube Poster', 'new-video-gallery' ); ?></option>
					</optgroup>
				</select>
		...
		public function _vg_save_settings( $post_id ) {
			if ( isset( $_POST['vg_save_nonce'] ) ) {
				if ( isset( $_POST['vg_save_nonce'] ) || wp_verify_nonce( $_POST['vg_save_nonce'], 'vg_save_settings' ) ) {
					...
					update_post_meta( $post_id, $awl_video_gallery_shortcode_setting, base64_encode( serialize( $gallery_settings ) ) );
				}
			}
